...
def main():
    ...
    kernel_fast = gpytorch.kernels.MaternKernel(nu=4.5).double()
    kernel_ref = gpytorch.kernels.MaternKernel(nu=4.5).double()
    ...
    loss_fast.backward()
    grad_fast = kernel_fast.raw_lengthscale.grad.clone()

    # Reference path: x requires grad, so this should use ordinary PyTorch.
    x_ref = x.clone().requires_grad_(True)
    covar_ref = kernel_ref(x_ref).to_dense()
    loss_ref = covar_ref.sum()
    loss_ref.backward()
    grad_ref = kernel_ref.raw_lengthscale.grad.clone()
